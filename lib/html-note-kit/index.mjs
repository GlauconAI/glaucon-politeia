...
function assertArtifactOutputSize(html, outputPath) {
  const bytes = Buffer.byteLength(html, "utf8");
  if (bytes > ARTIFACT_RESOURCE_LIMITS.artifactBytes) {
    fail(
      "ARTIFACT_OUTPUT_TOO_LARGE",
      "Generated artifact exceeds the maximum byte size",
      {
        bytes,
...
function assertDistinctImplicitOutput(manifestPath, outputPath) {
  let aliases = outputPath === manifestPath;
  if (!aliases) {
    try {
      const manifestStats = lstatSync(manifestPath);
      const outputStats = lstatSync(outputPath);
      aliases =
        manifestStats.dev === outputStats.dev && manifestStats.ino === outputStats.ino;
    } catch (cause) {
      if (cause?.code !== "ENOENT") {
        fail(
          "INVALID_BUILD_OPTIONS",
          "Unable to verify that implicit output differs from the manifest",
...
async function renderAndWriteInteractive({
  ...
  const bytes = assertArtifactOutputSize(html, outputPath);
...
export async function buildInteractiveArtifact(options) {
  ...
  if (inspected.outputPath === undefined) {
    assertDistinctImplicitOutput(manifestPath, outputPath);
  }
