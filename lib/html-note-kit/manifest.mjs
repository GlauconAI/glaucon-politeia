...
import { readTrustedUtf8 } from "./trusted-files.mjs";
...
async function importLocalModule(rootPath, source, code, label) {
  let before;
  try {
...
function loadCanonicalData(rootPath, definitions) {
  ...
  for (const definition of definitions) {
    const { content } = readTrustedUtf8(rootPath, definition.source, {
