...
function openTrustedFile(rootDirectory, source) {
  ...
  try {
    ...
    fail("UNSAFE_ENTRY_PATH", "Unable to open local entry safely");
  }
  ...
  let targetStats;
  try {
    openedStats = fstatSync(descriptor, { bigint: true });
    currentTarget = realpathSync(requestedPath);
    targetStats = statSync(currentTarget, { bigint: true });
  } catch {
    closeSync(descriptor);
    fail("UNSAFE_ENTRY_PATH", "Unable to verify opened local entry");
  }
...
function readStrictUtf8(descriptor, maximumBytes, code, entryName) {
...
function loadTrustedText(rootDirectory, source, maximumBytes, code, entryName) {
  const entry = openTrustedFile(rootDirectory, source);
  try {
    return {
      label: entry.label,
      content: readStrictUtf8(entry.descriptor, maximumBytes, code, entryName),
    };
  } finally {
    closeSync(entry.descriptor);
  }
...
const STANDALONE_IMPORT_TOKEN =
  /(?<![\p{ID_Continue}$\u200C\u200D])import(?![\p{ID_Continue}$\u200C\u200D])/u;

export function loadScriptEntry(rootDirectory, source) {
  const { label, content } = loadTrustedText(
    rootDirectory,
    source,
  ...
  if (
    STANDALONE_IMPORT_TOKEN.test(content) ||
...
function directChildrenByName(root, name) {
  return [...root.children].filter(
    (child) =>
      child.localName.toLowerCase() === name && isUnprefixedSvgElement(child),
  );
}
