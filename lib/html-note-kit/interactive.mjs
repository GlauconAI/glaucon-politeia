...
import { getManifestInternals } from "./manifest-registry.mjs";
import {
...
function inspectLoadedManifest(manifest) {
  ...
  let prototype;
  try {
    internals = getManifestInternals(manifest);
...
function invokeInternal(internals, method, code, message) {
  try {
...
export async function renderInteractiveModel(manifest, options = undefined) {
  ...
  for (const value of normalizedData.values()) {
