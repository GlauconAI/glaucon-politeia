...
function findClosingElement(lowerHtml, html, start, name) {
  const prefix = `</${name}`;
  let cursor = start;

  while ((cursor = lowerHtml.indexOf(prefix, cursor)) !== -1) {
...
function findScriptElements(html) {
  ...
  while ((cursor = html.indexOf("<", cursor)) !== -1) {
    ...
    if (tag.name === "script") {
      const closing = findClosingElement(lowerHtml, html, tag.end + 1, "script");
    ...
    if (RAW_TEXT_ELEMENTS.has(tag.name)) {
      const closing = findClosingElement(lowerHtml, html, tag.end + 1, tag.name);
