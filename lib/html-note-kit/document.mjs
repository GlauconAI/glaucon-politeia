...
function containsJsonDeclaration(html) {
  let fragment;
  try {
    fragment = JSDOM.fragment(html);
  } catch (cause) {
    fail(
      "INVALID_RENDERER_RESULT",
  ...
  while (pending.length > 0) {
    ...
    for (const script of root.querySelectorAll("script")) {
      if ((script.getAttribute("type") ?? "").trim().toLowerCase() === "application/json") {
        return true;
      }
    }
...
function normalizeSlots(value) {
  ...
  for (const key of SLOT_KEYS) {
    ...
    if (containsJsonDeclaration(slots[key])) {
      fail(
        "INVALID_RENDERER_RESULT",
        "Renderer slots must not declare application/json data",
        { slot: key },
      );
    }
...
function normalizeModel(value) {
  ...
  const result = {
    ...
    data,
    slots: normalizeSlots(inspected.slots),
