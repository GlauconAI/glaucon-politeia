...
function inspectSlotElements(html, dataIds) {
  try {
    ...
    while (pending.length > 0) {
      ...
      for (const element of root.querySelectorAll("[id]")) {
        ...
        if (dataIds.has(id)) return { collidingDataId: id };
      }
      for (const template of root.querySelectorAll("template")) {
        if (template.namespaceURI !== "http://www.w3.org/1999/xhtml") continue;
...
function normalizeSlots(value, dataIds) {
  ...
  for (const key of SLOT_KEYS) {
    ...
    const inspected = inspectSlotElements(slots[key], dataIds);
