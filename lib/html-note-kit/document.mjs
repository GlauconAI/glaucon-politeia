...
import { parse as parseHtmlDocument, parseFragment } from "parse5";
...
const PAGE_OVERFLOW_STYLE =
  "max-width: 100% !important; overflow-x: clip !important;";
const LAYOUT_OVERFLOW_STYLE =
  "min-width: 0 !important; max-width: 100% !important;";
const SVG_FRAME_OVERFLOW_STYLE =
...
function normalizeSlots(value, dataIds) {
  ...
  for (const key of SLOT_KEYS) {
    result[key] = protectSlotSvgFrames(result[key]);
  }
  return result;
}

function protectSlotSvgFrames(content) {
  ...
  while (pending.length > 0) {
    ...
    if (!(className ?? "").split(/[\t\n\f\r ]+/).includes("artifact-svg-frame")) {
      continue;
    }
    const currentStyle = attributeValue(node, "style");
    if (currentStyle === SVG_FRAME_OVERFLOW_STYLE) continue;
    const styleLocation = node.sourceCodeLocation?.attrs?.style;
    if (
      currentStyle !== undefined &&
      Number.isSafeInteger(styleLocation?.startOffset) &&
      Number.isSafeInteger(styleLocation?.endOffset)
    ) {
      const separator = currentStyle.trimEnd().endsWith(";") ? " " : "; ";
      edits.push({
        start: styleLocation.startOffset,
        end: styleLocation.endOffset,
        replacement: `style="${escapeHtml(`${currentStyle}${separator}${SVG_FRAME_OVERFLOW_STYLE}`)}"`,
      });
      continue;
    }
    const endOffset = node.sourceCodeLocation?.startTag?.endOffset;
    if (!Number.isSafeInteger(endOffset)) continue;
    const insertionOffset =
      content[endOffset - 2] === "/" ? endOffset - 2 : endOffset - 1;
    edits.push({
      start: insertionOffset,
      end: insertionOffset,
      replacement: ` style="${SVG_FRAME_OVERFLOW_STYLE}"`,
    });
  }
  edits.sort((left, right) => right.start - left.start);
  let protectedContent = content;
  for (const edit of edits) {
    protectedContent = `${protectedContent.slice(0, edit.start)}${edit.replacement}${protectedContent.slice(edit.end)}`;
  }
...
export function renderInteractiveDocument(model) {
  ...
  return render402vShell({
    lang,
    htmlAttributes: ` style="${PAGE_OVERFLOW_STYLE}"`,
    bodyAttributes: ` style="${PAGE_OVERFLOW_STYLE}"`,
...
${styleEntries}`,
    body: `  <div data-artifact-root style="${LAYOUT_OVERFLOW_STYLE}">
    <header class="artifact-topbar">
      <div class="artifact-topbar-inner" style="${LAYOUT_OVERFLOW_STYLE}">
    ...
    <div class="artifact-shell" style="${LAYOUT_OVERFLOW_STYLE}">
      ...
      <div class="artifact-layout" style="${LAYOUT_OVERFLOW_STYLE}">
        <main class="artifact-main-panel" style="${LAYOUT_OVERFLOW_STYLE}">
          ${slots.mainSections}
          <footer class="note-footer">${slots.footer}402v HTML Note Kit · standalone HTML</footer>
        </main>
        <aside class="artifact-rail" style="${LAYOUT_OVERFLOW_STYLE}" aria-label="Artifact information">
          ${slots.rail}
