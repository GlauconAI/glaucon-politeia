...
function slotBoundaries(slot, html) {
  let token = sha256(`${slot}\0${html}`);
  while (html.includes(token)) token = sha256(token);
  return {
...
function hasOnlyBoundaryAttribute(element, value) {
  return (
    ...
    element.getAttribute(SLOT_BOUNDARY_ATTRIBUTE) === value
  );
}

function inspectSlotElements(html, dataIds, parentElement, slot) {
  let dom;
  try {
    const boundary = slotBoundaries(slot, html);
    dom = new JSDOM(
      `<!doctype html><html><body><${parentElement} ${SLOT_BOUNDARY_ATTRIBUTE}="${boundary.wrapper}"><span ${SLOT_BOUNDARY_ATTRIBUTE}="${boundary.before}"></span>${html}<span ${SLOT_BOUNDARY_ATTRIBUTE}="${boundary.after}"></span></${parentElement}></body></html>`,
    );
    const { document } = dom.window;
    const wrappers = document.querySelectorAll(
      `[${SLOT_BOUNDARY_ATTRIBUTE}="${boundary.wrapper}"]`,
    );
    const beforeElements = document.querySelectorAll(
      `[${SLOT_BOUNDARY_ATTRIBUTE}="${boundary.before}"]`,
    );
    const afterElements = document.querySelectorAll(
      `[${SLOT_BOUNDARY_ATTRIBUTE}="${boundary.after}"]`,
    );
    if (
      wrappers.length !== 1 ||
