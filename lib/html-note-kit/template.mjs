...
export function render402vBaseStyles() {
  if (cached402vBaseStyles !== undefined) return cached402vBaseStyles;
  ...
  const end = document.indexOf(closing, start + opening.length);
  if (start === -1 || end === -1) {
    throw new Error("Unable to extract shared 402v base styles");
  }
  cached402vBaseStyles = document.slice(start + opening.length, end);
  return cached402vBaseStyles;
}
