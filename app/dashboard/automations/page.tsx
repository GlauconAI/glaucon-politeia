...
function filtersFrom(params: SearchParams): CronDirectoryFilters {
  return {
    view: oneOf(value(params, "view"), ["time", "agents", "all"] as const, "time"),
    q: value(params, "q") ?? "",
    owner: value(params, "owner") ?? "all",
    type: oneOf(
      value(params, "type"),
      ["all", "cron", "every", "at", "stream", "unknown"] as const satisfies readonly (
        | "all"
...
export default async function AutomationsPage({
  ...
  if (!currentAdmin) {
    redirect("/auth?redirectTo=/dashboard/automations");
  }
    ...
    <section className="observatory-page dashboard-directory-page">
      ...
      {state.status === "ready" ? (
        <CronDirectory
          ...
          projectionFrom={source?.collected_at ?? automations[0]?.collectedAt}
